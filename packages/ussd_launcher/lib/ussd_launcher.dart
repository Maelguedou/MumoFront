...
class UssdLauncher {
  ...
  /// Launches a USSD code with the system call UI and lets the user continue
  /// the session manually.
  ///
  /// Unlike [multisessionUssd], this method does not send menu options, does
  /// not hide dialogs, and does not cancel the USSD session automatically.
  static Future<void> launchUssdCall({
    required String code,
    required int slotIndex,
  }) async {
    try {
      await _channel.invokeMethod('launchUssdCall', {
        'ussdCode': code,
        'slotIndex': slotIndex,
      });
    } on PlatformException catch (e) {
      print("UssdLauncher: Error launching USSD call: ${e.message}");
      rethrow;
    }
  }
