...
class UssdMultiSession(private val context: Context) {
    ...
    fun launchUssdCall(str: String, simSlot: Int, result: Result) {
        if (str.isEmpty()) {
            result.error("EMPTY_USSD_CODE", "USSD code is empty", null)
            return
        }
        ...
        try {
            ...
            result.error("DIAL_ERROR", "Failed to launch USSD call: ${e.message}", null)
        }
    }
